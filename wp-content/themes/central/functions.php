...
function central_slider_page() { 
	...
	<div class="wrap">
		...
		<form method="post" action="admin.php?page=central_slider">
			...
			<span class="central-tips" style="font-size: 0.85em; color: #999;"><?php _e( "( for displaying a slides you need set a status 'draft' for posts of chosen category which you want to see in slider, featured images of posts you'll select will be displayed in the slider, an optimal featured image size for a slider is 900*218 pixels )", 'central' ); ?></span><br/>
					...
					<?php while ( $wp_query->have_posts() ) : $wp_query->the_post(); ?>
						<li><!-- show featured image, title and text-content of slides -->
							<?php if ( has_post_thumbnail() ) :
								...
								<?php add_filter( 'the_content', function ( $content ) { return preg_replace( '/<img[^>]+./', '', $content ); } );
								the_content();
								remove_filter( 'the_content', function ( $content ) { return preg_replace( '/<img[^>]+./', '', $content ); } );
...
if ( ! function_exists( '_wp_render_title_tag' ) ) :
	function central_render_title() { ?>
		<title><?php wp_title( '|', true, 'right' ); ?></title>
	<?php }
	add_action( 'wp_head', 'central_render_title' );
endif;
